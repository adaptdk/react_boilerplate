...
const _cssModuleLoader = isProd => [
  {
    ...
    options: {
      sourceMap: !isProd,
      modules: {
        localIdentName: isProd ? 's[hash:base64:3]' : '[name]__[local]--[hash:base64:3]',
      },
    },
  },
];
...
const sassLoader = isProd => ({
  ...
  use: [isProd ? _extractCssPlugin(isProd) : 'style-loader', 'css-loader', ..._postcssConfig, ..._sassConfig],
});
...
const cssLoader = isProd => ({
  ...
  use: [isProd ? _extractCssPlugin(isProd) : 'style-loader', 'css-loader', ..._postcssConfig],
});

// CSS Module Loader (CSS Modules)
const cssModuleLoader = isProd => ({
  test: cssModuleRegex,
  use: [isProd ? _extractCssPlugin(isProd) : 'style-loader', ..._cssModuleLoader(isProd), ..._postcssConfig],
});

// Collect and export
module.exports = {
  stylesLoaders: isProd => [sassLoader(isProd), sassModuleLoader(isProd), cssLoader(isProd), cssModuleLoader(isProd)],
};
